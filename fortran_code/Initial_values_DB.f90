...
!
! DESCRIPTION:
!   Initializes transition path arrays with steady-state values. Sets period 1-2
!   from old steady state (_ss_1), periods 3-bigT from new steady state (_ss_2).
!   Provides starting guess for the iterative transition path solver.
!
! INCLUDED IN: transition.f90 (before main iteration loop)
!===============================================================================

! from the old steady state
    k(1) = k_ss_1
    r_low(1) = r_low_ss_1
    asset_income_j(:,:,1) = asset_income_ss_j_1
    asset_base_j(:,:,1) = asset_base_ss_j_1
    l_j(:,:,1) = l_ss_j_1
    lab_j(:,:,1) = lab_ss_j_1
    c_j(:,:,1) = c_ss_j_1
    sv_j(:,:,1) = s_ss_j_1
...
do i = 3, bigT, 1 ! n_p+2,bigT,1
    k(i) = k_ss_2
    r_low(i) = r_low_ss_2
    asset_income_j(:,:,i) = asset_income_ss_j_2
    asset_base_j(:,:,i) = asset_base_ss_j_2
    l_j(:,:,i) = l_ss_j_2
    lab_j(:,:,1) = lab_ss_j_2
    c_j(:,:,i) = c_ss_j_2
    sv_j(:,:,i) = s_ss_j_2
...
do i = 2,2, 1 !n_p+1,1
    k(i) = k_ss_1
    r_low(i) = r_low_ss_1
    asset_income_j(:,:,i) = asset_income_ss_j_1
    asset_base_j(:,:,i) = asset_base_ss_j_1
    l_j(:,:,i) = l_ss_j_1
    c_j(:,:,i) = c_ss_j_1
    sv_j(:,:,i) = s_ss_j_1
    b_j(:,:,i) = b_ss_j_1
