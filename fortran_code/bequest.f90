...
if (switch_unequal_bequest==0) then       
    do i = 1,bigT,1
        do j = 2,jbar_t(i),1
             bequest_left_j(j-1,max(i-1,1)) = (N_t_j(j-1,max(i-1,1)) - N_t_j(j,i))*r(i)*sv_j(j-1,max(i-1,1))/(gam_t(i))   
        enddo
        do j = jbar_t(i)+1,bigJ,1
            bequest_left_j(j-1,max(i-1,1)) = (N_t_j(j-1,max(i-1,1)) - N_t_j(j,i))*(r(i)*sv_j(j-1,max(i-1,1)))/(gam_t(i))
        enddo
        bequest_left_j(bigJ,max(i-1,1)) = (N_t_j(bigJ,max(i-1,1)))*(r(i)*sv_j(bigJ,max(i-1,1)))/(gam_t(i))    
    enddo
                 
    bequest = sum(bequest_left_j(1:bigJ,:), dim=1)
...
elseif (switch_unequal_bequest==1) then
      do i = 1,bigT,1
        do j = 2,jbar_t(i),1
             bequest_left_j(j-1,max(i-1,1)) = (N_t_j(j-1,max(i-1,1)) - N_t_j(j,i))*sv_j(j-1,max(i-1,1))   
        enddo
        do j = jbar_t(i)+1,bigJ,1
            bequest_left_j(j-1,max(i-1,1)) = (N_t_j(j-1,max(i-1,1)) - N_t_j(j,i))*sv_j(j-1,max(i-1,1))
        enddo
        bequest_left_j(bigJ,max(i-1,1)) = (N_t_j(bigJ,max(i-1,1)))*(sv_j(bigJ,max(i-1,1)))    
      enddo
      
    do i = 1,bigT,1               
    bequest(i) = sum(bequest_left_j(1:bigJ,max(i-1,1)), dim=1)
    enddo
    bequest_j(:,:) = 0d0
    endif