...
!
  ...
  subroutine interpolation(endo_grid, egzo_grid, f, g, n_a, n_b)
  integer ( kind = 4 ) n_a, n_b
  integer ( kind = 4 ) ind( (n_a + 1)*(n_b+1))
  integer ( kind = 4 ) nod_tri(3,2* (n_a + 1)*(n_b+1))
  integer ( kind = 4 ) tnbr(3,2* (n_a + 1)*(n_b+1))
  integer ( kind = 4 ) tri_num
  integer ( kind = 4 ) ierror
  real    ( kind = 8 ) endo_grid(0:n_a, 0:n_b), egzo_grid(0:n_a, 0:n_b), f(0:n_a, 0:n_b), g(0:n_a, 0:n_b)
  ...
  ! reshape - check that
  ! we need to rewrite endo_gid in such a manner that we have matrix with dim (2:(n_a + 1)*(n_b+1))
  
  ! we need to start with  triangulation of endo_grid
  call dtris2 ((n_a + 1)*(n_b+1), endo_grid, ind, tri_num, nod_tri, tnbr, ierror )
  
  ! we need to locate each point of egzo_grid in triangles define above
  
  
  ! create weights to each point (a_w, b_w, c_w)
  
  
  ! define g bese on f and triangulation results
