...
check_pension_clearing = 0.0d0
        ...
        do m = 1,bigM,1
            do j = 1,bigJ,1
                check_pension_clearing = check_pension_clearing +  N_big_ss_j(j,m)*t1_ss*w_bar_ss(m)*l_ss_j(j,m)
            enddo
        enddo
...
gini_val_lab_pret       = gini(vec_lab_pretax_inc,vec_prob)
    ...
    if (switch_run_1 == 1) then    
        ...
        sum_b_weight_vec_ss_old = sum_b_weight_vec_ss
        do m = 1,bigM,1
        ...
        sum_b_weight_trans_outer(2:) = sum_b_weight_ss
        do i= 2,bigT,1
            do m = 1,bigM,1
...
if (switch_run_1 == 1) then    
    ...
    do j = 1,bigJ,1
        if (j == 1) then
            ...
            mult_ss = mult_ss + delta**(j-1)*pi_ss(j)/pi_ss(1)
        endif
    enddo
