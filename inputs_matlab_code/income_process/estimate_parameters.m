...
if ~isempty(n_reps_env)
    ...
    n_reps_source = 'environment variable N_REPS';
elseif exist(config_file, 'file')
    config_text = strtrim(fileread(config_file));
    token = regexp(config_text, '^\s*(\d+)\s*$', 'tokens', 'once');
    if isempty(token)
        error('N_REPS config file must contain a single non-negative integer: %s', config_file);
    end
    n_reps = str2double(token{1});
    n_reps_source = config_file;
else
    n_reps = 0;
    n_reps_source = 'default';
end
if isnan(n_reps) || n_reps < 0 || n_reps ~= floor(n_reps)
    error('N_REPS must be a non-negative integer. Current value: %s', num2str(n_reps));
end
fprintf('Using n_reps = %d (%s)\n', n_reps, n_reps_source);
...
for ivar = 1:length(variants)
    ...
    for itype = 1:length(types)
        ...
        if opt.bootstrap == 1
            ...
            % Fail early with an actionable message if MATLAB is configured
            % for bootstrap but the Stata bootstrap stage was not run with
            % the same N_REPS setting.
            for c = 1:p.C_eff
                sample_cov_file = fullfile(data_dir, ...
                    [type '_cohort' num2str(year((c-1)*5+1)) '_rep1.txt']);
                sample_nobs_file = fullfile(data_dir, ...
                    ['Nobs_' type '_cohort' num2str(year((c-1)*5+1)) '_rep1.txt']);
                if ~exist(sample_cov_file, 'file') || ~exist(sample_nobs_file, 'file')
                    error(['N_REPS=%d requests MATLAB bootstrap estimation, but bootstrap covariance files are missing. ', ...
                        'Run the Stata PSID stage after setting the same N_REPS value in ', ...
                        'inputs_stata_code/income_process/N_REPS.txt (or in the N_REPS environment variable), ', ...
                        'then rerun this MATLAB stage. For the standard fast replication, set N_REPS=0; ', ...
                        'plot_estimates.m will use the packaged *_archive.mat files for the paper confidence bands. ', ...
                        'First missing file checked: %s'], opt.bootstrap_rep, sample_cov_file);
                end
            end
