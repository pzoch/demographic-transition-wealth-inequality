...
import delimited  $resultspath\\`scenario'\\mass_trans_small.csv, clear delimiters(";") varnames(1)
	drop if inc_shock==6
	keep if mass > 0
...
graph export $graphspath\\Results_Gini_counterfactuals.eps, replace 
graph export $graphspath\\Results_Gini_counterfactuals.svg, replace 


global variant_base "psid_all_govt__" 
global variant_comp "psid_ndm_govt__ psid_nds_govt__ psid_ndo_govt__ "   
global legend `"legend(order(1 "[S1] Longevity:1955" 2 "[S1a] ExpectedLongevity:1955" 3 "[S1b] PopStructure:1955" ) cols(3) size(*0.8))"'
...
graph export $graphspath\\Results_Gini_drivers_demographics.eps, replace 
graph export $graphspath\\Results_Gini_drivers_demographics.svg, replace 

global variant_base "psid_all_govt__" 
global variant_comp "psid_ndm_govt__ psid_nlb_govt__ psid_ntx_govt__ psid_nts_govt__ "  
global legend `"legend(order(1 "[S1] Longevity:1955" 2 "[S2] IncomeINEQ:1955" 3 "[S3] Taxes:1955" 4 "[S4] Technology:1955") cols(2) size(*0.8))"'
...
graph export $graphspath\\Results_Gini_drivers_comparison.eps, replace 
graph export $graphspath\\Results_Gini_drivers_comparison.svg, replace 
...
if !_rc {
	...
	display as text "Raw ACS/IPUMS extract not found; keeping packaged college-share inputs and existing college_share graph."
	display as text "To regenerate Figure B.1, place ACS_college.dta in inputs_stata_code/skill_premium/ACS_college/ and rerun this driver."
}
...
global legend `"legend(order(1 "[S2] IncomeINEQ:1955" 2 "[S2a] CollegeShare:1955 " 3 "[S2b] CollegePremium:1955" 4 "[S2c] Shocks:Initial" ) cols(3) size(*0.8))"'
...
graph export $graphspath\\Results_Gini_drivers_incomes.eps, replace 
...
global legend `"legend(order(1 "[S3] Taxes:1955" 2 "[S3a] {&tau}{sub:L}:1955 " 3 "[S3b] {&tau}{sub:C}:1955" 4 "[S3c] {&tau}{sub:K}:1955" 5 "[S3d] Progressivity:1955" ) cols(3) size(*0.8))"'
...
graph export $graphspath\\Results_Gini_drivers_taxes.eps, replace 
...
global legend `"legend(order(1 "[S4] Technology:1955" 2 "[S4a] LabShare:1955" 3 "[S4b] TFP:1955" 4 "[S4c] Depr:1955" ) cols(3) size(*0.8))"'
...
graph export $graphspath\\Results_Gini_drivers_macro.eps, replace
...
graph export $graphspath\\AppF_Gini_counterfactuals_theta.eps, replace 
...
graph export $graphspath\\AppF_Gini_counterfactuals_hetrates.eps, replace 
graph export $graphspath\\AppF_Gini_counterfactuals_hetrates.svg, replace 
...
graph export $graphspath\\AppF_Gini_counterfactuals_homogendelta.eps, replace 
graph export $graphspath\\AppF_Gini_counterfactuals_homogendelta.svg, replace 
...
graph export $graphspath\\AppF_Gini_counterfactuals_nosuperstars.eps, replace 
graph export $graphspath\\AppF_Gini_counterfactuals_nosuperstars.svg, replace 
...
foreach scratch in bone.dta bone1y.dta {
    capture confirm file "$bsource`scratch'"
    if !_rc {
        capture shell attrib -R "$bsource`scratch'"
        capture erase "$bsource`scratch'"
    }
}
cd ..\outputs_stata_code
...
graph export $graphspath\\AppF_Gini_counterfactuals_gamma.eps, replace 
...
graph export $graphspath\\AppF_Gini_counterfactuals_exograte.eps, replace
...
graph export $graphspath\\AppF_Gini_counterfactuals_beq.eps, replace
