...
| **§14-P1d — `outputs_stata_code/data/*.dta` missing** | **resolved** in `c5`: frozen `irr_data.dta` / `benefits_cbo.dta` / `avghours_data.dta` committed under `outputs_stata_code/data/`. MvD_1 gains a `capture mkdir data` safety net. `c6` adds `outputs_stata_code/_bootstrap_MvD_data.do` as the Stata-based regenerator (extracted from MvD_1's three download blocks, gated on `$download_data=1`, default no-op). A `.stpr`-interactive run of the bootstrap regenerates the three `.dta` files from dbnomics when needed. The initial `.dta` snapshots in `c5` were produced via an interim Python script (now removed) because the `dbnomics` Stata package hit Mata compile errors on the author's current Stata 16 install (`urlencode()` / `libjson` class faults) even after `ssc install libjson; ssc install moremata`; the files are Stata-readable and were validated end-to-end via a `MvD_1` batch run. | §14.5-P3 |
